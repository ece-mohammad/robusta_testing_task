...
class TestLogin:
    ...
    def test_missing_username_expect_error_message(self, password):
        login = LoginPage(self.driver)
        login.open()
        login.login("", password)
        assert login.error_message() == "Epic sadface: Username is required"

    def test_missing_password_expect_error_message(self, username):
        login = LoginPage(self.driver)
        login.open()
        login.login(username, "")
        assert login.error_message() == "Epic sadface: Password is required"

    def test_wrong_username_expect_error_message(self, password):
        login = LoginPage(self.driver)
        login.open()
    ...
    def test_wrong_password_expect_error_message(self, username):
        login = LoginPage(self.driver)
        login.open()
